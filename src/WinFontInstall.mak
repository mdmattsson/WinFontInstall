...
BASE_NAME=WinFontInstall


!IF "$(ARCH)" != "x32" && "$(ARCH)" != "x64"
!MESSAGE Available build configurations:
!MESSAGE NMAKE /f "$(BASE_NAME).mak" ARCH="x32"
!MESSAGE NMAKE /f "$(BASE_NAME).mak" ARCH="x64"
!MESSAGE NMAKE /f "$(BASE_NAME).mak" ARCH="x32" USER="true"
!MESSAGE NMAKE /f "$(BASE_NAME).mak" ARCH="x64" USER="true"
...
!IF "$(USER)" == "true"
_USER_PRIV_FLAG=1
BASE_NAME=$(BASE_NAME)User-$(ARCH)
OBJDIR=$(OBJDIR)-user
!else
BASE_NAME=$(BASE_NAME)-$(ARCH)
!ENDIF
...
LINK_OBJS_1= \
	"$(OBJDIR)\WinFontInstall.obj" \
	"$(OBJDIR)\ReadFontName.obj" \
	"$(OBJDIR)\GetArgv.obj" \
	"$(OBJDIR)\SimpleString.obj" \
	"$(OBJDIR)\WinFontInstall.res"
...
!IF "$(USER)" == "true"
CPPFLAGS_DEF_EX=/D "_USER_PRIV_FLAG"
!ENDIF
...
!IF $(MAKE_DOPOST) > 0
	 @cd "$(BINDIR)"
	-@Certutil -hashfile $(BASE_NAME).exe MD5 1>"$(BASE_NAME).md5"
	 @cd ..
!ENDIF
